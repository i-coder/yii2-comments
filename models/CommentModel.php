...
class CommentModel extends ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            ['status', 'default', 'value' => CommentStatus::BLOCK],
            ['status', 'in', 'range' => [CommentStatus::ACTIVE, CommentStatus::DELETED,CommentStatus::BLOCK]],
    ...
    public function validateParentID($attribute)
    {
        if ($this->{$attribute} !== null) {
            $comment = self::find()->where(['id' => $this->{$attribute}, 'entity' => $this->entity, 'entityId' => $this->entityId])->exists();
//            $comment = self::find()->where(['id' => $this->{$attribute}, 'entity' => $this->entity, 'entityId' => $this->entityId])->active()->exists();
    ...
    {
        return ArrayHelper::map(self::find()->joinWith('author')->all(), 'createdBy', 'author.first_name');
    }
